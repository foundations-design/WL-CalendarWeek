...
Module.register("MMM-CalendarWeek", {
    ...
    getDom: function () {
        ...
        for (var e in events) {
            ...
            for (day in upcommingDays) {
                ...
                //Single day events
                    ...
                    event.fullDayEvent &&
                    moment(day).isBetween(startMoment, endMoment)
                ) {
                    upcommingDays[day].push(event);
                }
            }
        ...
        for (day in upcommingDays) {
            ...
            /*Add "empty" class if no events*/
            if (events.length < 1) {
                col.classList.add("empty");
            }

            var dateAsString = moment(moment(day), "x").format(
            ...
            for (var e in events) {
                    ...
                    if (event.fullDayEvent) {
                        ...
                        if (this.config.showEndDate) {
                            timeWrapper.innerHTML =
                                timeWrapper.innerHTML +
                                "</br>" +
                                moment(event.endDate, "x").format(timeFormatString);
                        }
                ...
                if (this.config.displayLocation && event.location) {
                    ...
                    locationString.classList.add("location");
        ...
        if (
            this.config.calendarTitle != false &&
            this.config.calendarTitle.length > 0
        ) {
    ...
    isMultiDayEvent: function (event) {
        return !moment(event.endDate, "x").isSame(
